...
$(document).ready(function() {
  ...
  $('.spin-button').click(function() {
    spins++;
    let result = Math.floor(Math.random() * categories.length);
    loadQuestion(categories[result].category);

    let totalDegree = 61 * (result + 1);
    totalDegree += spins * 180 * categories.length;
...
async function load() {
  let allowOrigin = 'https://api.allorigins.win/raw?url=';
  if (
    $('#txtUrlQuestion').val().includes('localhost') ||
